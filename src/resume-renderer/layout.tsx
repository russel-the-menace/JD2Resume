...
export async function runCanonicalLayout(snapshot: RenderSnapshot, signal: AbortSignal): Promise<{ pagePlan: PagePlanV2; report: LayoutReportV2 }> {
  ...
  let bestUsable: { pagePlan: PagePlanV2; attempt: number; score: number; failureCode: string; validation: ReturnType<typeof validateRenderedDom> } | null = null;
  for (const [index, strategy] of PUPPET_TUNING_STRATEGIES.entries()) {
    ...
    try {
      ...
      if (!bestUsable || score < bestUsable.score) bestUsable = { pagePlan: candidate.plan, attempt: index + 1, score, failureCode: failureCodes[0], validation };
    } catch (error) {
