...
import { DEFAULT_LAYOUT_FONT_SIZE_PX, DEFAULT_LAYOUT_LINE_HEIGHT_PX, DEFAULT_LAYOUT_TITLE_MARGIN_BOTTOM_PX, DEFAULT_LAYOUT_TITLE_MARGIN_TOP_PX } from './constants';
...
function tuningStyle(tuning: LayoutTuningV2, accent: string, layout?: RenderSnapshot['document']['data']['layout']) { return { '--resume-accent': accent, '--layout-section-gap-delta': `${tuning.sectionGapDelta}px`, '--layout-line-height-delta': `${tuning.lineHeightDelta}px`, '--layout-font-size-delta': `${tuning.fontSizeDelta}px`, '--resume-line-height': `${layout?.lineHeight ?? DEFAULT_LAYOUT_LINE_HEIGHT_PX}px`, '--resume-title-margin-top': `${layout?.titleMarginTop ?? DEFAULT_LAYOUT_TITLE_MARGIN_TOP_PX}px`, '--resume-title-margin-bottom': `${layout?.titleMarginBottom ?? DEFAULT_LAYOUT_TITLE_MARGIN_BOTTOM_PX}px`, '--resume-font-size': `${layout?.fontSize ?? DEFAULT_LAYOUT_FONT_SIZE_PX}px` } as CSSProperties; }
...
export function PuppetMeasurementDocument({ snapshot, tuning }: Props) {
  return <article className="puppet-measurement-document" style={tuningStyle(tuning, snapshot.document.accent, snapshot.document.data.layout)} lang={snapshot.document.language === 'chinese' ? 'zh-CN' : 'en'}>{buildRendererBlocks(snapshot.document).map((block) => <PuppetBlock key={block.id} block={block} gapBefore={Number(block.gapBeforeToken) || 0} tuneGap />)}</article>;
}
