...
function tuningStyle(tuning: LayoutTuningV2) { return { '--layout-section-gap-delta': `${tuning.sectionGapDelta}px`, '--layout-line-height-delta': `${tuning.lineHeightDelta}px`, '--layout-font-size-delta': `${tuning.fontSizeDelta}px` } as CSSProperties; }
...
export function PuppetMeasurementDocument({ snapshot, tuning }: Props) {
  return <article className="puppet-measurement-document" style={tuningStyle(tuning)} lang={snapshot.document.language === 'chinese' ? 'zh-CN' : 'en'}>{buildRendererBlocks(snapshot.document).map((block) => <PuppetBlock key={block.id} block={block} gapBefore={Number(block.gapBeforeToken) || 0} tuneGap />)}</article>;
}
export function PuppetPaginatedDocument({ snapshot, tuning, pagePlan }: Required<Props>) {
  ...
  return <main className="puppet-document" data-renderer-version={pagePlan.rendererVersion} data-snapshot-hash={pagePlan.snapshotHash} data-page-count={pagePlan.pages.length} style={tuningStyle(tuning)} lang={snapshot.document.language === 'chinese' ? 'zh-CN' : 'en'}>
