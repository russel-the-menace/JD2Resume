...
export function ResumePreviewFrame({ document, revision, onValidPlan, allowContentRefinement = false, onLayoutFailure }: { document: RendererResumeDocument; revision: number; onValidPlan: (pagePlan: PagePlanV2, report: LayoutReportV2) => void; allowContentRefinement?: boolean; onLayoutFailure?: (report: LayoutReportV2) => void }) {
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const contentRefinedRef = useRef(false);
  const renderer = useResumeRenderer({ document, revision, iframeRef, onValidPlan, onLayoutFailure: (_hash, report) => { if (!allowContentRefinement || contentRefinedRef.current) return; contentRefinedRef.current = true; onLayoutFailure?.(report); } }); const pages = renderer.status === 'ready' ? renderer.pagePlan?.pages.length || 1 : 1;
