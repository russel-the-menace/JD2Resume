...
export function ResumePreviewFrame({ document, lastPlan, onValidPlan }: { document: RendererResumeDocument; lastPlan: PagePlanV2 | null; onValidPlan: (pagePlan: PagePlanV2, report: LayoutReportV2) => void }) {
  const iframeRef = useRef<HTMLIFrameElement>(null); const signature = useMemo(() => JSON.stringify(document), [document]); const [revision, setRevision] = useState(1);
  useEffect(() => setRevision((current) => current + 1), [signature]);
  const renderer = useResumeRenderer({ document, revision, iframeRef, onValidPlan }); const visiblePlan = renderer.status === 'ready' ? lastPlan : null; const pages = visiblePlan?.pages.length || 1;
