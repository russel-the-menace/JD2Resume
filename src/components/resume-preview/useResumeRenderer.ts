...
export function useResumeRenderer({ document, revision, iframeRef, onValidPlan }: { document: RendererResumeDocument; revision: number; iframeRef: React.RefObject<HTMLIFrameElement | null>; onValidPlan: (pagePlan: PagePlanV2, report: LayoutReportV2) => void }) {
  ...
  useEffect(() => {
    const receive = async (event: MessageEvent<RendererToEditorMessage>) => {
      ...
      if (event.data.kind === 'RENDER_FAILED') { setStatus('failed'); setFailureCode(event.data.failureCode); return; }
  ...
  }, [iframeRef, onValidPlan, postRender]);
